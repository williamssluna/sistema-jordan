...
# 4.1. Cámara "Ojo de Halcón" (Máxima precisión con trucos de imagen)
def scan_pos(image):
    if not image: return None
    try:
        # Convertimos la imagen recibida a un formato leíble por OpenCV
        file_bytes = np.asarray(bytearray(image.read()), dtype=np.uint8)
        img = cv2.imdecode(file_bytes, 1)
        
        # Arsenal de filtros para forzar la lectura del código de barras
        ...
        # Probamos todas las versiones de la imagen en distintos ángulos
...
# 4.2. Carga rápida de listas (Categorías y Marcas)
def load_data(table):
    try:
...
# 4.3. Motor de Agregado al Carrito de Ventas (Usado por escáner y modo manual)
    ...
    try:
        ...
        if prod_db.data:
            ...
            if p['stock_actual'] > 0:
                ...
                st.error("❌ Sin stock disponible en tienda.")
        else: 
            st.warning("⚠️ Producto no encontrado en inventario.")
    except Exception as e: 
        st.error(ERROR_ADMIN)
    return exito

# ==========================================
# 5. ESTRUCTURA DE LA PÁGINA (CABECERA Y MENÚ LATERAL)
# ==========================================
st.markdown('<div class="main-header">📱 ACCESORIOS JORDAN | SMART POS v6.5</div>', unsafe_allow_html=True)

st.sidebar.markdown("### 🏢 Panel de Control")

# Filtro de Menú según Seguridad (Vendedor vs Administrador)
...
# Módulo de Autenticación en la Barra Lateral
...
if menu == "🛒 VENTAS (POS)":
        ...
        with st.expander("📷 ABRIR ESCÁNER TÁCTIL", expanded=False):
            ...
            if img:
                ...
                if code:
                    ...
                    st.error("⚠️ Foto muy borrosa. Intenta darle más luz.")
    ...
    with col_v2:
        ...
        if not st.session_state.carrito: 
            ...
            total_venta = 0
            # Módulo de Regateo Dinámico
            for i, item in enumerate(st.session_state.carrito):
                st.write(f"**{item['cant']}x** {item['nombre']} (Mín Permitido: S/. {item['p_min']:.2f})")
            ...
            if st.button("🏁 PROCESAR PAGO", type="primary"):
                ...
                try:
                    ...
                    # 3. Generar Ticket y Limpiar Carrito
        ...
        if st.session_state.last_ticket:
            with st.container():
                ...
                # Formato Ticket Interno (Muestra Ganancias, solo uso del dueño)
                if tk['doc'] == "Ticket Interno":
                    ...
                    <b>UTILIDAD NETA: S/. {tk['total'] - total_costo:.2f}</b><br>
                    MÉTODO: {tk['pago']}<br>
                    --------------------------------<br>
                    </div>
                    """
                
                # Formato Ticket Cliente (Limpio y Seguro)
                else:
                    for it in tk['items']: 
...
# ==========================================
    ...
    # PESTAÑA 1: INGRESO DE PRODUCTOS
    with t1:
        st.markdown('<div class="css-card">', unsafe_allow_html=True)
        
        # Lector de Ingreso con Alerta de Duplicados
        with st.expander("📷 ABRIR ESCÁNER ALMACÉN", expanded=True):
            ...
            if img_a:
                ...
                if code_a: 
                    ...
                    st.error("⚠️ La foto está muy borrosa. Intenta darle más luz.")
                ...
                if c_cod and c_nom and not cats.empty and not mars.empty:
                    try:
                        ...
                        if check_exist.data:
                            ...
                            # Guardamos en BD incluyendo el stock_inicial para cuadre
                            supabase.table("productos").insert({
                                ...
                                "stock_inicial": f_stock
                            }).execute()
                            
                            st.session_state.iny_alm_cod = "" 
                            exito_guardar = True
                    except Exception as e: 
                        st.error("Error al guardar. Asegúrate de rellenar todo correctamente.")
                else: 
    ...
    # PESTAÑA 2: CONFIGURACIÓN DE LISTAS
    with t2:
    ...
    # PESTAÑA 3: INVENTARIO Y REABASTECIMIENTO
    with t3:
        ...
        try:
            ...
            if prods.data: 
                ...
                # Rescate de stock_inicial para evitar errores
                ...
                if st.button("➕ Sumar al Stock Físico", type="primary"):
                    ...
                    if selected_prod != "Seleccionar...":
                        ...
                        try:
                            # Se actualizan ambos stocks para mantener la lógica contable
                            supabase.table("productos").update({
                                "stock_actual": c_stk + add_stock, 
                                "stock_inicial": c_ini + add_stock
                            }).eq("codigo_barras", cod_up).execute()
                            exito_re = True
                        except Exception as e: 
                            st.error(ERROR_ADMIN)
                        
                        if exito_re:
                            st.success(f"✅ Stock actualizado. Nuevo stock total: {c_stk + add_stock}")
                            time.sleep(1.5); st.rerun() 
            else: 
                st.info("📭 Aún no se han registrado productos en el inventario.")
        except Exception as e: 
            pass # Silenciamos error si la BD está vacía al inicio
        
    # PESTAÑA 4: HISTORIAL DE MERMAS DEL TURNO
    with t4:
        ...
        try:
            # Obtener fecha de último cierre para filtrar
            try:
                c_db = supabase.table("cierres_caja").select("*").order("fecha_cierre", desc=True).limit(1).execute()
                lc_dt = pd.to_datetime(c_db.data[0]['fecha_cierre'], utc=True) if c_db.data else pd.to_datetime("2000-01-01T00:00:00Z", utc=True)
            except: 
                lc_dt = pd.to_datetime("2000-01-01T00:00:00Z", utc=True)
            ...
            if mermas_db.data:
                ...
                # Filtro: Solo mostrar mermas creadas DESPUÉS del último cierre
                ...
                if not df_m_filt.empty:
                    ...
                    st.success("✅ Historial visual limpio. No hay mermas registradas en este turno.")
            else:
                st.success("✅ Historial completamente limpio en la base de datos.")
        except Exception as e:
            pass
...
# ==========================================
    ...
    with st.expander("📷 ESCANEAR PRODUCTO A DEVOLVER", expanded=False):
        ...
        if img_dev:
            ...
            if code_dev:
                ...
                st.warning("⚠️ No se detectó código. Intenta enfocar mejor.")
    ...
    if search_dev:
        if "AJ-" in search_dev.upper():
            # Flujo 1: Búsqueda por Ticket Completo
            try:
                    ...
                    for d in v_det.data:
                        ...
                        if col_d2.button("Ejecutar Devolución", key=f"dev_{d['id']}"):
                            ...
                            try:
                                ...
                                # Registrar impacto negativo en caja
                                supabase.table("devoluciones").insert({
                                    "producto_id": d['producto_id'], 
                                    "cantidad": d['cantidad'], 
                                    "motivo": "Devolución por Ticket", 
                                    "dinero_devuelto": d['subtotal'], 
                            ...
                            if exito_dev: 
                                st.success("✅ Dinero descontado de caja y producto vuelto a vitrina."); time.sleep(1.5); st.rerun()
                else: 
                    st.warning("⚠️ Ticket no encontrado en el sistema histórico.")
            except Exception as e: 
                st.error(ERROR_ADMIN)
            
        else:
            # Flujo 2: Búsqueda Manual Directa por Producto
            try:
                ...
                if p_db.data:
                    ...
                    with st.form("form_dev_libre", clear_on_submit=True):
                        ...
                        # Input vital para que la caja cuadre según el regateo original
                    ...
                    st.warning("⚠️ El código ingresado no corresponde a ningún producto registrado.")
            except Exception as e: 
                st.error(ERROR_ADMIN)

# ==========================================
# ⚠️ MÓDULO 4: MERMAS Y DAÑOS (CON TARJETA DE CONFIRMACIÓN)
# ==========================================
    ...
    if m_cod:
        try:
            # Buscamos primero para confirmar qué producto se va a botar
...
# ==========================================
    ...
    # --- VISTA 1: ACABA DE CERRAR CAJA (MUESTRA TICKET Z) ---
    if st.session_state.ticket_cierre:
        tk = st.session_state.ticket_cierre
        st.success("✅ Cierre de caja procesado correctamente. Contadores de vistas reiniciados a cero.")
        
        ticket_z_html = f"""
        <div class="ticket-termico">
            <center><b>ACCESORIOS JORDAN</b></center>
            <center>REPORTE DE CIERRE (TICKET Z)</center>
            --------------------------------<br>
            FECHA CIERRE: {tk['fecha']}<br>
            --------------------------------<br>
            <b>💰 RESUMEN DE VENTAS:</b><br>
            Cant. Productos Vendidos: {tk['cant_vendida']}<br>
            Total Ingresos Brutos: S/. {tk['tot_ventas']:.2f}<br>
            --------------------------------<br>
            <b>🔄 RESUMEN DE DEVOLUCIONES:</b><br>
            Cant. Productos Devueltos: {tk['cant_devuelta']}<br>
            Total Dinero Reembolsado: S/. {tk['tot_dev']:.2f}<br>
            --------------------------------<br>
            <b>⚠️ RESUMEN DE MERMAS:</b><br>
            Cant. Productos Dañados: {tk['cant_merma']}<br>
            Impacto (Pérdida Capital): S/. {tk['tot_merma']:.2f}<br>
            --------------------------------<br>
            <b>🏦 CUADRE FINAL DE CAJA:</b><br>
            EFECTIVO NETO ESPERADO: S/. {tk['caja_neta']:.2f}<br>
            GANANCIA PURA DEL TURNO: S/. {tk['utilidad']:.2f}<br>
            --------------------------------<br>
            <center>Stock Físico Mantenido con Éxito</center>
            <center>Iniciando Nuevo Turno</center>
        </div>
        """
        st.markdown(ticket_z_html, unsafe_allow_html=True)
        
        if st.button("🧹 Limpiar Pantalla e Iniciar Nuevo Turno", type="primary"):
            st.session_state.ticket_cierre = None
            st.rerun()

    # --- VISTA 2: MONITOREO DEL TURNO ACTUAL ---
    else:
        try:
            # 1. Obtener la barrera de tiempo (Último Cierre)
            try:
                ...
                st.caption(f"⏱️ Mostrando operaciones exclusivas desde el último cierre: {last_cierre_dt.strftime('%d/%m/%Y %H:%M')}")
            except:
                last_cierre_dt = pd.to_datetime("2000-01-01T00:00:00Z", utc=True)

            # 2. Extraer toda la data contable
            ...
            total_ventas_brutas, total_costo_vendido, total_devoluciones, total_perdida_mermas = 0.0, 0.0, 0.0, 0.0
            cant_vendida, cant_devuelta, cant_merma = 0, 0, 0
            df_rep_filtered = pd.DataFrame()
            
            # Procesamiento Matemático del Turno: VENTAS
            if detalles.data:
            ...
            # Procesamiento Matemático del Turno: DEVOLUCIONES
            if devs.data:
            ...
            # Procesamiento Matemático del Turno: MERMAS
            if mermas.data:
            ...
            c3.markdown(f"<div class='metric-box'><div class='metric-title'>Caja Neta (Dinero Físico)</div><div class='metric-value metric-green'>S/. {caja_neta_real:.2f}</div></div>", unsafe_allow_html=True)
            
            st.write("")
            st.markdown("##### 📈 Rendimiento del Turno")
            c4, c5 = st.columns(2)
            c4.markdown(f"<div class='metric-box'><div class='metric-title'>Pérdidas (Mermas)</div><div class='metric-value metric-red'>- S/. {total_perdida_mermas:.2f}</div></div>", unsafe_allow_html=True)
            c5.markdown(f"<div class='metric-box'><div class='metric-title'>Utilidad Neta (Ganancia)</div><div class='metric-value metric-green'>S/. {ganancia_neta_real:.2f}</div></div>", unsafe_allow_html=True)
            
            # --- FORMULARIO DE CORTE DE CAJA ---
            st.markdown('<div class="cierre-box">', unsafe_allow_html=True)
            st.write("### 🛑 CORTAR CAJA (FIN DE TURNO)")
            st.write("Esta acción sellará las operaciones contables del día, generará el Ticket Z y dejará las pantallas en blanco para iniciar un nuevo día. **El inventario físico permanecerá intacto.**")
            
            with st.form("form_cierre", clear_on_submit=True):
                clave_cierre = st.text_input("Ingresa clave Admin para confirmar", type="password")
                if st.form_submit_button("🔒 CONFIRMAR CIERRE Z", type="primary"):
                    if clave_cierre == "123456":
                        try:
                            # 1. Guardar en Base de Datos Histórica Cierres
                            ...
                            # 2. Generar el Ticket Z en Memoria para imprimirlo
                            ...
                            st.error("🚨 Error crítico. Por favor ejecuta el código SQL en Supabase primero para crear la tabla de cierres_caja.")
                    else:
                        st.error("❌ Clave incorrecta. Solo el administrador puede autorizar el corte.")
            st.markdown('</div>', unsafe_allow_html=True)

            st.divider()
            
            # Tabla Detallada Visual del Turno
            st.write("### 📝 Detalle de Ventas del Turno")
            if not df_rep_filtered.empty:
